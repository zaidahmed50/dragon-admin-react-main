...
export const useVendorData = () => {
    ...
    const fetchVendors = React.useCallback(async (query, page) => {
        setLoading(true);
        setError(null);
        try {
            ...
            if (query) {
                ...
                data = data.filter(item => 
                    ...
                    (item.vendorCode && item.vendorCode.toLowerCase().includes(lowerQuery)) ||
                    (item.contactPerson && item.contactPerson.toLowerCase().includes(lowerQuery)) ||
    ...
    }, [pageSize, currentPage, fetchVendors, searchQuery]);
